const EVENT_DECK = [
    ...
    },
    {
        ...
        effect: (game) => {
            ...
            Object.entries(game.abilities || {}).forEach(([color, action]) => {
                if(["attack", "sabotage", "invisibleSabotage"].includes(action.ability)) {
            ...
            game.players.filter(p => p.alive).forEach(player => {
                if(!hostileActions.has(player.color)) {
                    changes.push({ color: player.color, auraChange: 1, reason: "Solidarity event" });
                }
            });
            return changes;
        }
    },
    {
        ...
        effect: (game) => {
            ...
            if(!hostAction || hostAction.ability !== "defend") {
                changes.push({ color: host, auraChange: -2, reason: "Betrayal event - didn't defend" });
            }
            return changes;
        }
    },
    {
        id: "healing-circle",
        name: "Healing Circle",
        description: "All heal actions this round are guaranteed successful (+1 Aura to healer)",
    ...
    },
    {
        ...
        effect: (game) => {
            const aliveDoomerCount = game.players.filter(p => p.alive && p.role === "doomer").length;
    ...
    },
    {
        id: "double-down",
        name: "Double Down",
        description: "Vote outcomes this round have double the Aura impact (±1 Aura)",
    ...
    },
    {
        ...
        effect: (game) => {
            ...
            changes.push({ color: lowest.color, auraChange: 2, reason: "Power Shift - lowest" });
            if(lowest.color !== highest.color) {
                changes.push({ color: highest.color, auraChange: -1, reason: "Power Shift - highest" });
            }
            return changes;
        }
    },
