...
const RightPanels = ({ playerCharacter, otherPlayers = [], myPlayer = null }) => {
  ...
  return (
    <div className="absolute top-5 right-5 bg-white/10 p-5 rounded-3xl flex flex-col gap-0 max-h-[90vh] overflow-y-auto">
      <h3 className="text-white text-xl font-bold mb-2">All Players</h3>

      {allPlayers.map((player, index) => (
        <div key={index} className="p-2 flex gap-5 border-b border-white/20 last:border-b-0">
          <img
          ...
          <div>
            <p className="text-white text-lg">
              Color: <span className="font-bold capitalize">{player.color || player.id}</span>
            </p>
            {player.isHost && (
