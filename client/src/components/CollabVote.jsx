...
const CollabVote = ({ playerColor, onClose}) => {
    ...
    const handleVoteClick = (proposer) => {
        setPendingVote(proposer);
    };
    ...
    >
        ...
        >
            {proposers.map((proposer) => {
                const votedForThis = votes[playerColor] === proposer;
                const isPending = pendingVote === proposer;

                return (
                    <div
                        key={proposer}
                        onClick={() => handleVoteClick(proposer)}
                    ...
                    <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            <img
                            src={`/images/charactersHead/${proposer}.png`}
                            className="w-12 h-12 rounded-full"
                            />
                            <p className="text-lg font-semibold opacity-80">{proposer}</p>
                        </div>

                        {/* Voters */}
                        <div className="flex flex-wrap gap-2 pl-14">
                            {voters
                            .filter(voter => votes[voter] === proposer)
                            .map(voter => (
                                <img
                                key={voter}
