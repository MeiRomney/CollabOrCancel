...
const Results = () => {
  ...
  const doomersRemained = alivePlayers.filter(
    (p) => p.role === "doomer",
  ).length;
  ...
  return (
    <div style={backgroundStyle}>
      <div className="pt-5 my-auto flex flex-col items-center">
        ...
        <p
          className={`text-5xl font-bold mt-10 ${isDoomersWin ? "text-red-600" : isVibersWin ? "text-green-600" : "text-white"}`}
        >
          {isDoomersWin
            ? "DOOMERS WIN!"
            : isVibersWin
      ...
      <div className="absolute top-20 left-5 flex flex-col items-center justify-center">
        ...
        <div className="mt-5 bg-transparent border border-white border-2 p-5 rounded-3xl flex flex-col gap-0">
          {winners.slice(0, 4).map((winner) => {
            const player =
              allPlayers.find((p) => p.color === winner.color) || winner;
            return (
              <div key={winner.color} className="p-2 flex gap-5">
                <img
                ...
                <div>
                  <p className="text-white text-3xl">
                    Color:{" "}
                    <span className="font-bold capitalize">{winner.color}</span>
                  </p>
                  <p className="text-white text-3xl">
                    Role:{" "}
                    <span className="font-bold capitalize">{winner.role}</span>
                  </p>
                  <p className="text-white text-2xl">
                    Aura:{" "}
                    <span className="font-bold">
                      {player.aura?.toFixed(1) || 0}
                    </span>
                  </p>
                </div>
              </div>
      ...
      <div className="absolute top-20 right-5 flex flex-col items-center justify-center">
        ...
        <div className="mt-5 bg-white/10 p-5 rounded-3xl flex flex-col gap-0">
          {allDoomers.slice(0, 3).map((doomer) => {
            return (
              <div key={doomer.color} className="p-2 flex gap-5">
                <img
                ...
                <div>
                  <p className="text-white text-xl">
                    Color:{" "}
                    <span className="font-bold capitalize">{doomer.color}</span>
                  </p>
                  <p className="text-white text-xl">
                    Status:{" "}
                    <span className="font-bold">
                      {doomer.alive ? "Alive" : "Eliminated"}
                    </span>
                  </p>
                </div>
              </div>
        ...
        <div className="relative mt-5 p-5 rounded-3xl bg-transparent border-white border-2 flex flex-col justify-center items-center">
          ...
          <p className="text-2xl text-white mx-auto">
            Doomers Remained:{" "}
            <span className="font-bold">{doomersRemained}</span>
          </p>
        </div>
      </div>

      {/* Buttons */}
      <div className="absolute bottom-10 left-1/4 flex gap-10 mt-10">
        <button
